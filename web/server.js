...
app.post('/player/upload', function (req, res) {
    ...
    busboy.on('file', (fieldname, file, filename, encoding, mimetype)=>{
        ...
        .catch(e=>res.status(500).send(JSON.stringify({success: false, error: e.toString()}))));
    req.pipe(busboy);
})

app.get('/player/songs', function (req, res) {
    music.listFiles().then(v=>{
        res.send(JSON.stringify({success: true, value: v}))
    }).catch(e=>res.send(JSON.stringify({success: false, error: e})));
})
